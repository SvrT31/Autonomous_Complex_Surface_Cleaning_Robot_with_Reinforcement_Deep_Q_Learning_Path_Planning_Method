...
def create_graph(custom_numbers):
    graph = {}
...
def closest_point(custom_numbers, robot_x, robot_y):
    graph = create_graph(custom_numbers)
    distances = dijkstra(graph, (robot_x, robot_y))
    min_distance = float('inf')
    closest_point = None
    
    for point, distance in distances.items():
        if distance < min_distance and point in custom_numbers:
