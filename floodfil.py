...
def flood_fill(robot_x, robot_y, custom_numbers):
    visited = set()
    ...
    while queue:
        ...
        if (x, y) in custom_numbers:
            decrease_custom_number(custom_numbers, x, y)

        visited.add((x, y))

        neighbors = [(x+1, y), (x-1, y), (x, y+1), (x, y-1)]
        for nx, ny in neighbors:
            if (nx, ny) not in visited and (nx, ny) in custom_numbers:
                queue.append((nx, ny))
...
while running:
    ...
    if custom_numbers:
        ...
        if closest:
            robot_x, robot_y = closest
            cost = custom_numbers[closest]
            flood_fill(robot_x, robot_y, custom_numbers)
            step += cost
